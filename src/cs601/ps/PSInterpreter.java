...
public class PSInterpreter {
	...
	public void popAndPrint() {
        PSObject pso = operandStack.pop();
        if(pso instanceof PSOperator) System.out.println(pso.toString());
        else System.out.println(pso);
    }
	...
	public void define(String key, PSObject value) {
        PSDictionary psd = dictionaryStack.pop();
        psd.put(key, value);
        dictionaryStack.push(psd);
	}
	...
	public PSObject lookup(String key) {
        int size = dictionaryStack.size();
        for(int i=size-1; i>=0; i--) {
            PSDictionary psd = dictionaryStack.get(i);
            PSObject pso = psd.get(key);
            if(pso != null) return pso;
        }
